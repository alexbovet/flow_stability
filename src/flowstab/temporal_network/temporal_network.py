...
from .temporal_network_internal import _TemporalNetwork
...
class TemporalNetwork:
    ...
    def __init__(self,
        ...
        if not self._use_as_is:
            ...
            #       labels
            _events_table = pd.DataFrame(data=data, columns=columns)

            # sorting according to start times
            _events_table.sort_values(
            ...
            self.nodes = np.sort(pd.unique(_events_table[
                [self._SOURCES, self._TARGETS]
            ].values.ravel("K"))).tolist()

        self._temporal_network = _TemporalNetwork(
            events_table=_events_table
        )


        # TODO: make properties, fetch form _tempnet
    ...
    @property
    def nodes(self):
        return self._temporal_network.nodes
...
class TemporalInstantNetwork(TemporalNetwork):
    """
